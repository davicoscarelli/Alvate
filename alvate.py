...
class Generator:
    ...
    def face_detect(self, images):
        ...
        if not nosmooth: boxes = self.get_smoothened_boxes(boxes, T=5)
    ...
    def __init__(self, source):
        ...
        while True:
            try:
                ...
                question = input("What are you feeling? \n")

                text = openai.Completion.create(
                    model="text-davinci-003",
                    prompt="Write something (under 50 words) motivativational or encouraging about what I'm feeling: "+question,
                    temperature=0.5,
                    max_tokens=60,
                    top_p=1.0,
                ...
                filename = "audio%02d.wav" % num_generated
                sf.write(filename, generated_wav.astype(np.float32), synthesizer.sample_rate)
                num_generated += 1

                print("AI: ", text.choices[0].text)
                ...
                command = 'ffmpeg -y -i {} -i {} -strict -2 -q:v 1 {}'.format(audio_file, 'temp/result.avi', 'results/result_voice.mp4')
                subprocess.call(command, shell=platform.system() != 'Windows')
